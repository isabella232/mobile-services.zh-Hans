...
1. 单击 **[!UICONTROL 启用最新的应用程序报表]**.
   ...
   或者，您也可以选择单击 **[!UICONTROL 启用Mobile位置跟踪]** 或 **[!UICONTROL 为后台点击量启用旧版报告和归因]**.
...
如果 `CollectLifecycleData()` 在同一会话中调用了两次，则您的应用程序会在首次调用后的每次调用中报告崩溃。 SDK会在应用程序关闭时设置一个标志，指示成功退出。 如果未设置此标志， `CollectLifecyleData()` 报告崩溃。
...
要设置 *`products`* 在mobile SDK中，必须使用特殊语法。 有关更多信息，请参阅 [产品变量](/help/universal-windows/analytics/products.md).

## （可选）启用离线跟踪 {#section_955B2A03EB854742BDFC4A0A3C287009}

要在设备处于离线状态时存储点击，您可以在 [SDK方法](/help/universal-windows/c-configuration/methods.md) 文件。 在启用离线跟踪之前，请密切注意配置文件引用中描述的时间戳要求。

## 地理位置和目标点 {#section_BAD34A8DD013454DB355121316BD7FD4}

地理位置允许您测量位置数据（纬度/经度）和预定义的目标点。 每个 `TrackLocation` 调用发送：

* 纬度/经度和POI(如果在 `ADBMobileConfig.json` 配置文件)。
...
当设备位置被确定为在定义点的7000米半径内时， `a.loc.poi` 具有值的上下文数据变量 `San Francisco` 随一起发送 `TrackLocation` 点击。 安 `a.loc.dist` 上下文变量随定义坐标的距离（以米为单位）一起发送。
