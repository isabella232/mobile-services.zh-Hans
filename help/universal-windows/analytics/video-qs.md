...
Video measurement is described in detail in the [Measuring video and audio in Adobe Analytics](https://docs.adobe.com/content/help/en/media-analytics/using/media-overview.html) guide. 在所有 AppMeasurement 平台中，测量视频的大概过程都非常相似。此快速入门部分提供开发人员任务的基本概述以及代码示例。
...
* **a.media.segment**
   ...
   默认的视频数据收集方法会在以下时间点收集数据：视频开始（播放）、区段开始和视频结束（停止）。当访客开始观看时，Analytics 会在第一个区段开始时计数第一个区段查看次数。后续区段查看次数会在相应区段开始时进行计数。
...
* **a.contentType**
   ...
   收集由访客查看的内容类型相关数据。由视频测量发送的点击量将被分配内容类型“视频”。此变量不需要专门为视频跟踪而进行保留。通过使用此相同变量让其他内容报告内容类型，您可以分析访客在不同内容类型之间的分布。例如，您可以使用此变量通过“article”或“product page”之类的值标记其他内容类型。

   从视频测量的角度来看，内容类型允许您识别视频访客，从而计算视频转化率。
