---
description: 操作是您要衡量的应用程序中发生的事件。 每个操作均具有一个或多个对应的量度，每当发生事件时，这些量度的数量都会递增。例如，您可以跟踪新订阅、每次查看文章或每次完成某个级别时。 这些事件的相应度量配置为订阅、已读文章和已完成的级别。
seo-description: 操作是您要衡量的应用程序中发生的事件。 每个操作均具有一个或多个对应的量度，每当发生事件时，这些量度的数量都会递增。例如，您可以跟踪新订阅、每次查看文章或每次完成某个级别时。 这些事件的相应度量配置为订阅、已读文章和已完成的级别。
seo-title: 跟踪应用程序操作
...
操作是您要衡量的应用程序中发生的事件。 每个操作均具有一个或多个对应的量度，每当发生事件时，这些量度的数量都会递增。例如，您可以跟踪新订阅、每次查看文章或每次完成某个级别时。 这些事件的相应度量配置为订阅、已读文章和已完成的级别。
